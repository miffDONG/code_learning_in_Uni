...
def preprocess_data(cfg, dataset, category_df):
    ...
    preprocess_dataset_path = './preprocessed_dataset'
    if os.path.isdir(preprocess_dataset_path):
        preprocessed = load_from_disk(preprocess_dataset_path)
        logger.info('preprocess dataset...Successfully loaded')
    else:
        ...
        preprocessed.save_to_disk(preprocess_dataset_path)
        logger.info('preprocess dataset...Successfully saved for next train')
    return preprocessed
...
def tokenize_data(cfg, dataset, tokenizer):
    ...
    if os.path.isdir(tokenized_dataset_path):
        ...
        logger.info('tokenize dataset...Successfully loaded')
        print(tokenized[0:5])
    else:
        ...
        logger.info('tokenize dataset...Successfully saved for next train')
    return tokenized
...
def get_config(checkpoint_path):
    cfg = OmegaConf.load('/content/drive/MyDrive/특허분류/baseline/config/train.yaml')
    cfg.train.checkpoint_path = checkpoint_path  # override checkpoint_path with the command line argument
    return cfg
